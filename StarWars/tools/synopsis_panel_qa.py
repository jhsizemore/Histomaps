...
try:
    ...
    with urllib.request.urlopen(req,timeout=20) as r:
        ...
        probe['server']={'ok':True,'status':r.status,'cors':r.headers.get('Access-Control-Allow-Origin'),'title':page.get('title'),'extract_chars':len(page.get('extract','')),'extract_preview':page.get('extract','')[:240]}
...
def assert_synopsis(locator,label):
...
with sync_playwright() as p:
    ...
    assert_synopsis(page.locator('.record-synopsis'),'legends overview')
    activate(page,'.event',0)
