...
paragraph('Film and TV markers show principal story windows, not release dates or runtimes. Shared-date titles use one grouped card with every individual lockup retained; its anchor stays fixed at the shared date. Fine leaders link shifted labels to their true dates.',60,FOOT+73,750,21)
...
print(json.dumps({'file':str(out),'pixels':im.size,'bytes':out.stat().st_size,'content':{'canon_events':len(events),'screen_titles':len(raw_media),'lifelines':len(lives),'legends_events':sum(len(L['events']) for L in D['legends'].values())}}))
