...
`.github/workflows/starwars-generated-assets.yml` is the permanent build and verification pipeline for the poster and social preview. It first runs the website↔poster data audit, validates the interactive JavaScript, renders the 5,200 × 18,220 poster, renders the 1,200 × 630 social card, and verifies output dimensions plus canonical URL metadata.

Changes to timeline data, lifelines, insignia, title art, the starfield, poster inputs, or either renderer trigger the workflow. Pull requests build the outputs and upload them as review artifacts without modifying the branch. Pushes to `main` rebuild both outputs and commit them only when the generated files actually differ. The workflow can also be run manually with `workflow_dispatch`.
